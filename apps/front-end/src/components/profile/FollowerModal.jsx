...
export default function FollowerModal({title}) {
    ...
    const { loading, error, items, hasMore } = useInfiniteScrolling(usersList, pageNumber)
                ...
                <ModalContent>
                    {items.map((username, key, index) => {
                        if (items.length === index + 1) {
                            return <FollowerModalContentItem ref={lastItemElementRef} key={Math.random()} username={username} index={index} loading={loading}/>
                        } else {
                            return <FollowerModalContentItem key={Math.random()} username={username} index={index} loading={loading}/>
                        }
                    })}
